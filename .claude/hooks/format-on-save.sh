...
if [ "$FORMATTED" = false ] && [ -f "$ROOT/node_modules/.bin/eslint" ] && [ -f "$ROOT/.eslintrc.json" -o -f "$ROOT/.eslintrc.js" -o -f "$ROOT/.eslintrc.cjs" -o -f "$ROOT/eslint.config.js" ]; then
  case "$EXTENSION" in
    js|jsx|ts|tsx)
      npx eslint --fix "$FILE_PATH" >/dev/null 2>&1 && FORMATTED=true
      ;;
  esac
fi
