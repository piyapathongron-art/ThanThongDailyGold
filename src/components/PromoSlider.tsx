...
const PromoSlider = ({ images }: { images: string[] }) => {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden rounded-2xl md:rounded-3xl glass shadow-2xl">
      {slideCount === 0 ? (
        <div className="absolute inset-0 bg-linear-to-br from-accent-red to-background flex flex-col items-center justify-center p-6 md:p-12 text-center space-y-4 md:space-y-6">
          ...
          <div className="space-y-1 md:space-y-2">
            <h2 className="text-2xl md:text-4xl 2xl:text-7xl font-bold text-white uppercase tracking-widest">Promotion</h2>
            <p className="text-base md:text-xl 2xl:text-4xl text-slate-300">อัปโหลดรูปได้ที่หน้า Settings</p>
          </div>
        </div>
      ) : (
          ...
          {images.map((src, index) => (
            <div
              ...
              className={`h-1.5 md:h-2 transition-all duration-300 rounded-full ${
                index === currentIndex ? 'w-8 md:w-12 bg-primary' : 'w-1.5 md:w-2 bg-white/20'
              }`}
            />
          ))}
        </div>
